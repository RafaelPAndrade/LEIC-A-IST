...
int main (int argc, char** argv) {
	...
	int csz = (argc >= 9 ) ? parse_integer_or_exit(argv[8], "csz") : 0 ;
	...
	// Error checking
	if(N < 1 || tEsq < 0 || tSup < 0 || tDir < 0 || tInf < 0 || iteracoes < 1 || trab < 1 || csz < 0) {
		fprintf(stderr, "\nErro: Argumentos invalidos.\nLembrar que N >= 1, "
		        "temperaturas >= 0, iteracoes >= 1, trab >= 1 e csz >= 0\n\n");
		exit(EXIT_WRONG_ARGS);
	}
	...
	// - Threaded version (with message passing)
	result = simul_t_m(matrix, N+2, N+2, iteracoes, trab, csz);
	if (result == NULL) {
		printf("\nErro na simulacao.\n\n");
		exit(EXIT_SIMUL);
	}
